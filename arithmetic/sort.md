...
10. 快速排序
    ...
    3. 代码见：cn.fenqing.arithmetic.sort.QuickSort
        顾名思义，他是通过键值的各个位的值，将要排序的元素分配至某些“桶”中，达到排序的作用
    ...
    2. 思想
        1. 将所有比较数值统一为同样的数位长度，数位较短的数前面补零，然后，从最低位开始，
           依次进行依次排序，这样从最低位开始，依次进行依次排序，这样从最低位排序，一直到最高排序完成以后
           数列就变成一个有序序列。
           
        2. 细节：
            1. 现有数组[53, 3, 542, 748, 14, 214]
            2. 创建二维数组，长度为10（桶）
            3. 先把每个元素的各位取，然后看这个数字应该放在那个桶中
            4. 按照这个桶的顺序，取出值放入原来的数组中，将变成[542, 53, 3, 14, 214, 748]
            5. 然后比较十位取，重复上述操作（没有则补0），一直往上[3, 14, 214, 542, 748, 53]
            6. 再重复百位比较，则：[3, 14, 53, 214, 542, 748]
        3. 代码见：cn.fenqing.arithmetic.sort.RedixSort
    

13. 稳定与不稳定的概念：
    稳定是指数组中如果有两个相同的值，排序后他们的先后顺序仍然保持排序前的顺序
    不稳定则是不一定保证顺序相同
